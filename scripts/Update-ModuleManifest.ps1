...
if (-not $VersionTag) {
    throw "Version tag is required. Provide -VersionTag parameter."
}
...
# Extract release notes from GitHub event automatically.
$ReleaseNotes = $null
if ($env:GITHUB_EVENT_PATH) {
    Write-Host "GitHub event path found: $env:GITHUB_EVENT_PATH"
    if (Test-Path $env:GITHUB_EVENT_PATH) {
        Write-Host "Reading GitHub event file..."
        try {
        ...
        Write-Host "GitHub event file not found at path"
    }
} else {
    Write-Host "No GitHub event path available"
}
